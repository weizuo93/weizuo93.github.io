...
* 执行cmake命令可能会报错：
```
CMake Error at cmake/ccache.cmake:21 (message):
  Using ccache: no (Could not find find ccache.  To significantly reduce
  compile times for the 2nd, 3rd, etc.  build, it is highly recommended to
  install ccache.  To suppress this message, run cmake with
  -DENABLE_CCACHE=0)
```
解决：安装ccache（可以先尝试是否能通过`yum -y install ccache`安装ccache）。下载ccache源码、进行编译安装(亲测可用)：
```
(下载地址：https://www.samba.org/ftp/ccache/)
[root@0a1f58989af5 package]# wget https://www.samba.org/ftp/ccache/ccache-3.6.tar.gz
[root@0a1f58989af5 package]# tar -zxvf ccache-3.6.tar.gz
[root@0a1f58989af5 package]# cd ccache-3.6
[root@0a1f58989af5 ccache-3.6]# ./configure
[root@0a1f58989af5 ccache-3.6]# make
[root@0a1f58989af5 ccache-3.6]# make install
```
或者执行cmake时添加`-DENABLE_CCACHE=0`配置(未测试是否可行)：
```
cmake -DENABLE_CCACHE=0 ../
```
